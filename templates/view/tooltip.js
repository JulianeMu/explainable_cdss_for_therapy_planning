...
function show_tool_tip_pie(current_node) {
    ...
    tooltip_div.select('svg')
        ...
        .style('height', tooltip_div.style('height'));//'100%');//tooltip_div.style('height'))

    create_text_pie_tooltip(current_node, tooltip_div);

    var text_width = 0;
    tooltip_div.selectAll('text').each(function () {
    ...
    if (text_width < parseFloat(tooltip_div.style('height'))) {
        text_width = parseFloat(tooltip_div.style('height'));
    }


    //tooltip_div.style('width', text_width + 'px');
    //tooltip_div.select('svg').style('width', text_width + 'px');

    tooltip_div.select('svg')
        .style('width', '100%')
        .style('height', '100%');

    // tooltip_div.selectAll('*').each(function () {
    //     var current_translate = d3.select(this).attr('transform');
    //
    //     if (current_translate !== null) {
    //         var splitted_translation = current_translate.split(',');
    //         d3.select(this).attr('transform', 'translate(' + text_width/2 + ',' + parseFloat(splitted_translation[1]) + ')');
    //     }
    // });
